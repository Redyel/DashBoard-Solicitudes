...
aplazamientos = pd.read_excel("./Conflicto_Matricula.xlsx",engine='openpyxl')
...
ndf = df.assign(demora=0)[['Radicado','Fecha de Radicado','Fecha Seguimiento','Dependencia Seguimiento','demora' ,'Estado Solicitud','Sede','Cuál es el conflicto que desea solucionar','Usuario Seguimiento','Estado Seguimiento','Hora']]
...
xndf = ndf[['Radicado','Fecha de Radicado','Fecha Seguimiento','Dependencia Seguimiento','demora','Estado Solicitud','Sede','Cuál es el conflicto que desea solucionar','Usuario Seguimiento','Estado Seguimiento','Hora']]
xndf['demora'] = pd.to_timedelta(xndf['demora']).dt.days
print(xndf)
xndf.to_csv('Ok_Conflicto_Matricula.csv',encoding='latin1')