#Importar las liberias necesarias para el desarrollo
import pandas as pd
import openpyxl
import numpy as np
from datetime import datetime

aplazamientos = pd.read_excel("./Aplazamiento_Modulo.xlsx",engine='openpyxl')
...
ndf = df.assign(demora=0)[['Radicado','Fecha de Radicado','Fecha Seguimiento','Dependencia Seguimiento','demora' ,'Estado Solicitud','Sede en la que estudia actualmente','Motivo por el cual hace esta solicitud ','Usuario Seguimiento','Estado Seguimiento','Hora']]
...
cola = ndf['Radicado'].count()
xndf = ndf[['Radicado','Fecha de Radicado','Fecha Seguimiento','Dependencia Seguimiento','demora','Estado Solicitud','Sede en la que estudia actualmente','Motivo por el cual hace esta solicitud ','Usuario Seguimiento','Estado Seguimiento','Hora']]
xndf['demora'] = pd.to_timedelta(xndf['demora']).dt.days
print(xndf)
xndf.to_csv('Ok_Aplazamiento_Modulo.csv',encoding='latin1')